...
def epsilon_greedy(Q,
    ...
    rnd_s = np.random.rand()  # generate a random number
    if eps_type == 'constant':
        ...
        if rnd_s < epsilon:
            #action = np.random.randint(len(all_actions))
        ...
        # It is recommended you use the np.random module
        sche_linear = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = sche_linear.value(current_total_steps)
        if rnd_s < epsilon:
            action = np.random.randint(len(all_actions))
        else:
            action = np.nanargmax(Q[state])
