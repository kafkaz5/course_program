...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # It is recommended you use the np.random module
        obj = ScheduleLinear(anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = obj.value(current_total_steps)
        p = np.random.uniform(0, 1)
        if p < epsilon:
            action = np.random.choice(all_actions)
        else:
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold and R is not self.settings.rewards[0]:
            ...
            while not end_episode:
                ...
                action = epsilon_greedy(Q, s_current,
                               list_pos,
                ...
                next_move = np.nanargmax(Q[s_next])
                Q[s_current][action] = (1 - lr) * Q[s_current][action] + lr * (R + self.gamma * Q[s_next][next_move])
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
...
class ScheduleLinear(object):
    def __init__(self, schedule_timesteps, final_p=0.2, initial_p=1.0):
    ...
    def value(self, t):
        ...
        if t > self.schedule_timesteps:
            ...
            current_p = self.initial_p + delta_p * (t / self.schedule_timesteps)
            return current_p
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
