...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        #print(obj.value(current_total_steps))

        if np.random.uniform(0,1) < obj.value(current_total_steps):
            action = np.random.choice(all_actions)
        else:
            action = np.nanargmax(Q[state])
        # ADD YOUR CODE SNIPPET BETWEENEX  3.2
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                nextMove = np.nanargmax(Q[s_next])
                difference = (self.gamma * Q[s_next][nextMove] + R) - Q[s_current][action]
                Q[s_current][action] += self.alpha * difference
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        value = (self.final_p - self.initial_p) * (t / self.schedule_timesteps) + self.initial_p
        if value < self.final_p:
            return self.final_p
        return value
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
